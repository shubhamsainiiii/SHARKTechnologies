...
import {
    ...
    SiMongodb, SiPostgresql, SiMysql, SiRedis, SiFirebase, SiSupabase,
    SiDocker, SiKubernetes, SiVercel, SiNetlify,
    SiFlutter, SiExpo, SiAndroid, SiApple, SiGithubactions,
} from 'react-icons/si';
    ...
    return (
        <section className="section-padding" style={{ background: 'var(--bg-primary)' }}>
            <div className="container-custom">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    {techCategories.map((cat, catIdx) => (
                        <motion.div key={cat.category} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: catIdx * 0.08 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                                <span style={{ width: '30px', height: '30px', background: `${cat.color}15`, border: `1px solid ${cat.color}30`, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>{categoryIcons[cat.category]}</span>
                                <span style={{ fontFamily: 'Syne,sans-serif', fontWeight: '700', fontSize: '0.78rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: cat.color }}>{cat.category}</span>
                                <div style={{ flex: 1, height: '1px', background: `linear-gradient(90deg, ${cat.color}30, transparent)` }} />
                            </div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
                                {cat.techs.map((tech, techIdx) => {
                                    const icon = techIcons[tech] || { Icon: () => <span style={{ fontWeight: 700, fontSize: '0.8rem' }}>{tech[0]}</span>, color: cat.color };
                                    return (
                                        <motion.div key={tech} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: catIdx * 0.05 + techIdx * 0.04 }} whileHover={{ y: -3, transition: { duration: 0.15 } }}
                                            style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', cursor: 'default', transition: 'border-color 0.2s', width: '120px', flexShrink: 0 }}
                                            onMouseEnter={e => e.currentTarget.style.borderColor = `${cat.color}40`}
                                            onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}
                                        >
                                            <div style={{ width: '38px', height: '38px', background: `${icon.color}15`, border: `1px solid ${icon.color}25`, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', color: icon.color }}>
                                                <icon.Icon />
                                            </div>
                                            <span style={{ fontFamily: 'DM Sans,sans-serif', fontSize: '0.72rem', fontWeight: '500', color: 'var(--text-secondary)', textAlign: 'center', lineHeight: '1.3' }}>{tech}</span>
                                        </motion.div>
                                    );
                                })}
                            </div>
...
export default function ServicesPage() {
    ...
    useEffect(() => {
        fetch(`${API_URL}/services?status=Active&limit=50`)
            .then(r => r.json())
    ...
    return (
        <>
            {/* Hero */}
            <section style={{ paddingTop: '140px', paddingBottom: '60px', background: 'var(--bg-primary)', position: 'relative', overflow: 'hidden' }} className="grid-bg">
                <div style={{ position: 'absolute', top: '20%', right: '10%', width: '360px', height: '360px', background: 'radial-gradient(circle, rgba(0,212,255,0.07) 0%, transparent 70%)', pointerEvents: 'none' }} />
                <div className="container-custom" style={{ position: 'relative', zIndex: 1 }}>
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.2)', borderRadius: '50px', padding: '6px 16px', marginBottom: '20px', fontSize: '0.72rem', fontFamily: 'Syne,sans-serif', fontWeight: '700', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent)' }}>What We Offer</span>
                        <h1 style={{ fontFamily: 'Syne,sans-serif', fontWeight: '800', fontSize: 'clamp(1.8rem, 3.5vw, 2.8rem)', lineHeight: '1.08', letterSpacing: '-0.03em', color: 'var(--text-primary)', marginBottom: '16px', maxWidth: '640px' }}>
                            Services That Drive{' '}
                            <span style={{ background: 'linear-gradient(135deg, var(--accent), #00ffcc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Real Results</span>
                        </h1>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', lineHeight: '1.7', maxWidth: '480px' }}>
                            A full spectrum of digital services — from design to deployment. We own every layer of your product.
                        </p>
            ...
            <section className="section-padding" style={{ background: 'var(--bg-secondary)', paddingTop: '40px' }}>
                <div className="container-custom">
                    {loading && (
                        <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--text-muted)', fontFamily: 'Syne,sans-serif' }}>Loading services...</div>
                    )}
                    {!loading && services.length === 0 && (
                        <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--text-muted)', fontFamily: 'Syne,sans-serif' }}>No services found.</div>
                    )}
                    {!loading && services.length > 0 && (
                        <div className="services-grid">
                            {services.map((service, i) => {
                                const accent = service.accent || ACCENT_COLORS[i % ACCENT_COLORS.length];
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Title + tagline */}
                                        <h3 style={{ fontFamily: 'Syne,sans-serif', fontWeight: '700', fontSize: '1.05rem', color: 'var(--text-primary)', marginBottom: '4px', letterSpacing: '-0.01em' }}>{service.title}</h3>
                                        {service.tagline && (
                                            <p style={{ fontSize: '0.72rem', fontFamily: 'Syne,sans-serif', color: accent, fontWeight: '600', letterSpacing: '0.04em', marginBottom: '10px' }}>{service.tagline}</p>
                                        )}

                                        {/* Desc */}
                                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', lineHeight: '1.7', marginBottom: '16px', flex: 1 }}>{service.shortDesc}</p>

                                        {/* Feature pills */}
                                        {features.length > 0 && (
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
                                                {features.slice(0, 4).map(f => (
                                                    <span key={f} style={{ padding: '3px 9px', background: `${accent}0d`, border: `1px solid ${accent}20`, borderRadius: '20px', fontSize: '0.68rem', color: 'var(--text-secondary)', fontFamily: 'DM Sans,sans-serif' }}>{f}</span>
                                                ))}
                                                {features.length > 4 && (
                                                    <span style={{ padding: '3px 9px', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: '20px', fontSize: '0.68rem', color: 'var(--text-muted)' }}>+{features.length - 4} more</span>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        <Link to={`/services/${service.slug}`}
                                            style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 16px', background: `${accent}12`, border: `1px solid ${accent}28`, borderRadius: '10px', color: accent, textDecoration: 'none', fontFamily: 'Syne,sans-serif', fontWeight: '600', fontSize: '0.78rem', transition: 'all 0.2s', alignSelf: 'flex-start' }}
                                            onMouseEnter={e => { e.currentTarget.style.background = `${accent}20`; e.currentTarget.style.borderColor = `${accent}50`; }}
                                            onMouseLeave={e => { e.currentTarget.style.background = `${accent}12`; e.currentTarget.style.borderColor = `${accent}28`; }}
                                        >Learn More <FiArrowRight style={{ fontSize: '12px' }} /></Link>
                                    </motion.div>
                                );
                            })}
                        </div>
